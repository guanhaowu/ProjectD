...
def get_blinking_ratio(eye_points, facial_landmarks):
    ...
    hor_line_length = hypot(
        (left_point[0] - right_point[0]), (left_point[1] - left_point[1]))
...
while True:
    ...
    topCorner = (200, 100)  # (X, Y)
    bottomCorner = (400, 400)  # from 0,0 (x,y)

    roi = frame[topCorner[1]:bottomCorner[1], topCorner[0]: bottomCorner[0]]
    cv2.rectangle(frame, topCorner, bottomCorner,
                  (0, 200, 0), 5)  # search boundary rectangle
    ...
    for face in faces:
        ...
        apple_image = cv2.imread("apple.png")
        android_image = cv2.imread("android.png")

        if gaze_ratio <= 0.9:
            # RIGHT SIDE
            cv2.putText(frame, "Apple", (50, 150), font, 2, (128, 128, 128), 3)
            new_frame = apple_image
        elif 1 < gaze_ratio < 1.7:
            # CENTER
            cv2.putText(frame, "CENTER", (50, 100), font, 2, (0, 0, 255), 3)
        else:
            # LEFT SIDE
            cv2.putText(frame, "Android", (50, 150), font, 2, (0, 128, 0), 3)
            new_frame = android_image
